...
public class EnvironmentTestHelper : MonoBehaviour
{
    ...
    private IEnumerator AutoFlyRoutine()
    {
        ...
        float flySpeed = 104f;
        float bounceWavelength = 135f;
        float lastBounceZ = 0f;
        int skipCounter = 0;

        // 6. 3,500m 수면 관통 비행 루프
        while (currentDist < targetDist && isAutoFlying)
        {
            ...
            float stoneY = Mathf.Sin(wavePhase * Mathf.PI) * 2.2f + 0.15f;
            ...
            gc.stone.transform.position = new Vector3(spawnWorldPos.x, stoneY, spawnWorldPos.z + currentDist);
            ...
            if (currentDist - lastBounceZ >= bounceWavelength)
            {
                ...
                {
                    position = new Vector3(spawnWorldPos.x, 0.10f, spawnWorldPos.z + currentDist),
                ...
                {
                    SplashEffectSpawner.Instance.SpawnSplash(new Vector3(spawnWorldPos.x, 0.05f, spawnWorldPos.z + currentDist), 1.3f);
                }
