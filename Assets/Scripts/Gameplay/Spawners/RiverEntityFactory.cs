...
namespace SkippingStones.Gameplay.Spawners
{
    ...
    public class RiverEntityFactory
    {
        ...
        public void EnsurePrefabsLoaded()
        {
            ...
            string[] rockPaths = {
                "Assets/prefab/BG_Deco/P_BoulderClassic1 Variant.prefab",
                "Assets/prefab/BG_Deco/P_BoulderClassic2 Variant.prefab",
                "Assets/prefab/BG_Deco/P_BoulderClassic3 Variant.prefab",
                "Assets/prefab/BG_Deco/P_BoulderClassic4 Variant.prefab",
                "Assets/prefab/BG_Deco/P_BoulderClassic5 Variant.prefab"
            };

            for (int i = 0; i < 5; i++)
            {
                if (obstacleRockPrefabs[i] == null)
                {
                    obstacleRockPrefabs[i] = UnityEditor.AssetDatabase.LoadAssetAtPath<GameObject>(rockPaths[i]);
                }
            }
            ...
            for (int i = 0; i < 5; i++)
            {
                if (obstacleRockPrefabs[i] == null)
                {
                    obstacleRockPrefabs[i] = Resources.Load<GameObject>($"BoulderClassic_{i + 1}");
                }
            }
        ...
        public GameObject CreateObstacleRock(Transform parent, Vector3 pos)
        {
            ...
            if (obstacleRockPrefabs != null && obstacleRockPrefabs.Length > 0)
            {
                List<GameObject> valids = new List<GameObject>();
                for (int i = 0; i < obstacleRockPrefabs.Length; i++)
                {
                    if (obstacleRockPrefabs[i] != null) valids.Add(obstacleRockPrefabs[i]);
                }
                if (valids.Count > 0)
                {
            ...
            if (chosenPrefab == null && obstacleRockPrefab != null)
            {
                chosenPrefab = obstacleRockPrefab;
            }

            // 🌟 월드 Y축 랜덤 로테이션 적용 (0 ~ 360도)
            ...
            if (chosenPrefab != null)
            {
                ...
                if (rockComp == null)
                {
                    rockComp = obj.AddComponent<ObstacleRock>();
                }
                return obj;
            }
            else
