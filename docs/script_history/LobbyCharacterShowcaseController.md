...
### [2026-09-02] 4차 수정: 캐릭터 교체 트랜지션 시차(퇴장 완료 후 등장) 분리 및 카메라 밖 진입 보장
* **작업 내역**:
  1. `TransitionRoutine` 2단계 순차(Phase 1 퇴장 ➔ Phase 2 등장) 분리: 기존 캐릭터와 새 캐릭터가 동시에 엇갈려 겹치던 문제를 해결하고, 기존 캐릭터가 먼저 화면 밖으로 퇴장하여 완전히 삭제된 후 새 캐릭터가 스폰되어 중앙으로 걸어 들어오도록 연출 구조 혁신.
  2. 화면 밖 안전 거리(최소 2.2m) 보장: 카메라 화각 안에서 캐릭터가 갑자기 나타나거나 사라지지 않도록 2.2m 이상의 거리를 확보하고, 걷기 애니메이션 보유 캐릭터는 보폭 속도 비율에 맞춰 등장/퇴장 시간을 자동 최적화.
