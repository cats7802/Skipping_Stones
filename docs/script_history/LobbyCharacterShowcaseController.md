...
### [2026-09-01] 3차 수정: 로비 카메라 정면 시선 자동 정렬 및 셀렉트 애니메이션 연동
* **작업 내역**:
  1. `GetCameraFacingRotation(centerPos)` 구현: 로비 뷰 카메라(`Lobby_CAM`)의 위치를 자동 추적하여, 캐릭터 등장 및 안착 시 카메라 정면을 정확히 바라보도록 정렬(Y=73°).
  2. `PlaySelectAnimation()` 구현: 로비에서 [🚀 GO! (맵 선택)] 버튼 클릭 시 캐릭터가 환호(Select) 애니메이션을 재생하도록 연동.
* **컴파일 검증**: `0 Errors, 0 Warnings` 완료.
