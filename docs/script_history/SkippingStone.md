...
### [2026-08-30] 갓모드 바위 장애물(Rock/Obstacle) 통과 및 육지 지형 충돌 유지
- **수정 목적**: 갓모드 비행 시 강 한가운데 놓인 바위 장애물/데코 프랍에 부딪혀 끊기던 문제 해결 (지형 충돌 룰은 정상 보존).
- **핵심 구조**:
  - `OnCollisionEnter`, `OnTriggerEnter`, `CrashOnLand`에서 `isGodMode && isRock`일 때만 충돌 판정을 패스.
  - 강둑이나 육지 지형(`Terrain`, `Ground`, `Bank`) 충돌 시에는 기존과 동일하게 지형 착지 처리(원칙 준수).
- **컴파일 검증**: 0 Errors.
