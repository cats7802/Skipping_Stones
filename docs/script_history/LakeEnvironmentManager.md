...
### [2026-08-30] 모든 맵 프리팹 WaterSurface 필수 엄격 검증 탑재 & 레거시 수면 파괴 코드 완전 제거
- **수정 목적**: 
  1. 슬롯 맵 프리팹에서 `WaterSurface` 또는 수면 콜라이더가 누락되어 1000m 이상 구간에서 돌이 허공 추락하던 현상을 원천 방지하기 위해 등록 프리팹 전수 자동 검증 시스템 도입.
  2. 복제 청크 생성 시 `WaterSurface`를 강제 삭제하던 옛날 레거시 코드(`Destroy(wsc)`)를 완전 삭제하여 모든 청크의 수면 콜라이더가 정상 유지되도록 수정.
- **핵심 구조**:
  - `ValidatePrefabWaterSurface()`: `startMapPrefab`, `loopSlots`(베이스/변주), `endingMapPrefab`을 사전 검사하여 누락 시 명확한 에러 콘솔 로깅.
  - 투척 발판(`Platform`) 검증은 0번 시작 맵(`chunkIndex == 0`)에만 한정 적용(중간 루프 맵은 발판 불필요).
- **컴파일 검증**: 0 Errors.
