...
- **적용 내용**:
  - `StartLaunchLeadIn`: 55프레임 발사 예정 고정 앵커(`leadInAnchorPos`) 및 발사 방향(`leadInForwardDir`)으로 카메라 진행축(`smoothedFlightHeading`)을 100% 선동기화.
  - 45~55프레임 리드인 구간 동안 손의 돌을 일체 보지 않고, 55프레임 발사 대기 위치로 부드럽게 가속(`Lerp / Slerp`)하여 대기.
  - 55프레임 정식 발사(`DynamicFlight`) 시점에 0.001mm 단차 없이 즉시 비행 추적으로 매끄럽게 연결.
- **컴파일 검증**: 0 Errors.

### [2026-08-30] 초근접 밀착(Close-up) & 광각(High FOV) 비행 카메라 구도 적용
- **수정 목적**: 돌의 시각적 크기를 확대하면서도 배경 전경(굽이치는 강줄기, 산맥)의 공간감과 속도감을 대폭 확장.
- **적용 내용**:
  - `flightFOV = 75f` (기본 60도에서 비행 진입 시 75도로 다이내믹 확장).
  - 초근접 오프셋 적용: `flightDistBack = 2.8f`, `flightHeight = 1.25f`, `flightLookForward = 12.0f`, `flightLookHeight = -1.0f`.
  - 세로 9:16 화면 내 돌의 중앙 하단부 위치 보존 및 광각 파노라마 뷰 확보.
- **컴파일 검증**: 0 Errors.
